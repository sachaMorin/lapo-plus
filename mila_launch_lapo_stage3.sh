...
LR=$3
STEPS=$4
BC_ONLY=$5 # set to true for running just behavior cloning

# check if all arguments are provided
if [ -z "$SWEEP_NAME" ] || [ -z "$FREEZE_BACKBONE" ] || [ -z "$LR" ] || [ -z "$BC_ONLY" ]; then
  echo "Error: Missing argument(s). You should specifiy a sweep name, freeze_backbone flag, learning rate, and bc_only flag."
  echo "Usage: $0 <sweep_name> <freeze_backbone> <learning_rate> <bc_only>"
  exit 1
fi
...
for seed in "${SEEDS[@]}"; do
  for obs in "${OBSERVED_SAMPLES[@]}"; do
	# Run the train_lapo_stage3.py script with the current seed and observed samples
	python train_lapo_stage3.py env_name="${tasks[${ind}]}" exp_name="${exp_name}" seed="${seed}" lapo_stage3.n_observed_samples="${obs}" lapo_stage3.freeze_backbone=${FREEZE_BACKBONE} lapo_stage3.lr=${LR} lapo_stage3.bc_only=${BC_ONLY} lapo_stage3.steps=${STEPS} &&
	echo "Completed train_lapo_stage3 for env: ${tasks[${ind}]}, seed: ${seed}, observed samples: ${obs}"
  done
done
...
mv /tmp/wandb $SCRATCH/wandb_logs/${out}/${exp_name}
